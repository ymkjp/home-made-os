...
void HariMain(void)
{
	...
	char filebuf[512 * 1024], winbuf[1040 * 805 * osak_getbuflen()];
	char s[32], *p;
	...
	if (info[2] > sysinfo.scrnx || info[3] > sysinfo.scrny) {
	...
	if (bpp == 8) {
		...
		for (i = 0; i < info[3]; i++) {
			sp = (unsigned short *) (winbuf) + (i + 29) * xsize + (xsize - info[2]) / 2;
			q = picbuf + i * info[2];
			for (j  = 0; j < info[2]; j++) {
				sp[j] = (unsigned short)(((picbuf[i * info[2] + j].r << 8) & 0xf800) |
						((picbuf[i * info[2] + j].g << 3) & 0x07e0) | (picbuf[i * info[2] + j].b >> 3));
			}
		}
	}
